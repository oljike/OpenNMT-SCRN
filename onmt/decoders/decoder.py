...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _build_rnn(self, rnn_type, input_size,
        ...
        if rnn_type=="SCRN":
            stacked_cell = CustomStackSCRN
        elif rnn_type.lower()=='custom_lstm':
            stacked_cell = CustomStackLSTM
        elif rnn_type == "LSTM":
...
class CustomStackSCRN(nn.Module):
    def __init__(self, num_layers, input_size, hidden_size, dropout=0, context_size=40, alpha=0.95):
        super().__init__()
    ...
    def forward(self, inputs, state):
        ...
        hidden_state, context_state = state[0].squeeze(), state[1].squeeze()

        new_context = (1.0 - self.alpha) * torch.mm(inputs, self.B) + self.alpha * context_state

        new_hidden = torch.sigmoid(
            torch.mm(new_context, self.P) + torch.mm(inputs, self.A) + torch.mm(hidden_state, self.R) + self.bias_term)
...
class CustomStackLSTM(nn.Module):
    def __init__(self, num_layers, input_size, hidden_size, dropout=0):
        super().__init__()
        self.input_sz = input_size
        self.hidden_size = input_size
        self.weight_ih = Parameter(torch.randn(hidden_size * 4, input_size))
        self.weight_hh = Parameter(torch.randn(4 * hidden_size, hidden_size))
        self.bias_ih = Parameter(torch.randn(hidden_size * 4))
    ...
    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.weight_ih.size(1))
        self.weight_ih.data.uniform_(-stdv, stdv)
        self.weight_hh.data.uniform_(-stdv, stdv)
        if self.bias_ih is not None:
            self.bias_ih.data.uniform_(-stdv, stdv)
    ...
    def forward(self, x, hidden):
        ...
        h_t, c_t = hidden[0].squeeze(), hidden[1].squeeze()
        ### LSTM CELL
        ...
        return h_t, (h_t.unsqueeze(0), c_t.unsqueeze(0))
